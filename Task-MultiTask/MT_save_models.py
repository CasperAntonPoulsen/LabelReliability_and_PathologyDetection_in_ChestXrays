...
def PD_save_models(json_name, h5_name, lr=0.00001):
    ...
    model.compile(loss={"PD_output": tf.keras.losses.BinaryCrossentropy(), "TD_output": tf.keras.losses.BinaryCrossentropy()}, optimizer=adam, metrics=['accuracy', tf.keras.metrics.AUC(name='auc')])
...
for lr in learning_rate_list:
    ...
    model_acc1 = PD_save_models(json_name=path+"MT_model1.json", h5_name=path+"MT_model1.h5", lr=lr)
    model_acc2 = PD_save_models(json_name=path+"MT_model2.json", h5_name=path+"MT_model2.h5", lr=lr)
    model_acc3 = PD_save_models(json_name=path+"MT_model3.json", h5_name=path+"MT_model3.h5", lr=lr)
